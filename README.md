...
- 多线程能否共享一把锁：共享锁--独占锁
  - Java中ReentrantReadWriteLock读写锁，读锁为共享锁，写锁为独占锁
...
- 此类容器缺点：
  ...
  - 内存占用问题：因为CopyOnWrite的写时复制机制，所以写操作时候，内存里会同时驻扎两个对象的内存
#### BlockingQueue
...
- 主要方法：put-take阻塞；add-remove-element异常；offer-poll-peek忽略并返回false；
...
- 读写锁state的int值高16位存储共享锁，使用低16位存储独占锁
...
- AQS会维护一个等待线程的队列，把线程都放到这个队列里(双向链表)<br/>
